...
# Réponses attendues
reponseattendue1 = "KNN - Pour les valeurs entrées, la prédiction de stroke est : négative (risque de stroke non identifié)."
reponseattendue2 = "KNN - Pour les valeurs entrées, la prédiction de stroke est : positive (risque de stroke identifié)."
...
now = datetime.now()
r1 = requests.get(
        url='http://{address}:{port}/knnpred?username={username}&password={password}&age={age}&gluc={gluc}&bmi={bmi}&gendr={gendr}&smoking={smoking}'.format
...
output = '''=========================================================
    KNNPRED - Relevance test at {now}
=========================================================
   Request done at "/knnpred" with age={age1}, gluc={gluc1}, bmi={bmi1}, gendr={gendr1}, smoking={smoking1} :
      expected result = {reponseattendue1}
      actual result   = {text1}
      ==>  {test_status1}
   Request done at "/knnpred" with age={age2}, gluc={gluc2}, bmi={bmi2}, gendr={gendr2}, smoking={smoking2} :
